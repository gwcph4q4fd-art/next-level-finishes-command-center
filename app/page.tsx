...
export default async function DashboardPage() {
  const [openLeadCount, draftReplyCount, estimateDraftCount] = await Promise.all([
    prisma.lead.count({ where: { status: { in: ["NEW", "CONTACTED"] } } }),
    prisma.aiDraft.count({ where: { type: "LEAD_REPLY", status: "DRAFTED" } }),
    prisma.aiDraft.count({ where: { type: "ESTIMATE", status: "DRAFTED" } })
  ]);
      ...
      <div className="mx-auto flex w-full max-w-7xl gap-6 px-4 py-5 sm:px-6 lg:px-8">
        ...
        <div className="min-w-0 flex-1">
          ...
          <section className="mb-6 grid gap-4 lg:grid-cols-[1fr_1fr_1fr]">
            <Panel title="Today’s Priorities">
              <div className="grid gap-3 text-sm text-steel">
                <p>{openLeadCount} saved leads need attention, {draftReplyCount} reply drafts await approval, and {estimateDraftCount} estimate drafts are still open.</p>
                <Link className="font-semibold text-pine" href="/agent">Ask the AI agent what to do today</Link>
              </div>
            </Panel>
